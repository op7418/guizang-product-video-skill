...
编排 `plan.json` 时，分镜时长可按“信息摄入基础时间 + 视觉负荷”定量计算，再吸附到音乐小节：

`Duration = T_read + T_ui + T_buffer`

1. **阅读基础时间 (`T_read`)**：
   - 中文：按 7 到 9 字/秒计算。短标题按固定 0.5s，说明文案按 `字数 / 8` 计算。
   - 英文：按 3 到 4 词/秒计算。
2. **UI 认知负荷 (`T_ui`)**：
   - 纯大字报或口号：记 0s。
   - 单一按钮或输入框交互：记 1.0s。
   - 双图或分屏前后对比：记 1.5s。
   - 复杂代码、反汇编或架构拓扑：记 2.0s。
3. **视线落点缓冲 (`T_buffer`)**：固定取 0.5s，供眼睛定位与消化。

将计算得到的基础时长吸附到最近的偶数音乐小节（以常见的 120 BPM 为例）：
- **1 个小节（约 2 秒）**：适合纯告警、大字冲击或单行命令回车。
- **2 个小节（约 4 秒）**：适合绝大多数标准功能镜头（短标题 + 一句话说明 + 单组件）。
- **3 到 4 个小节（约 6 到 8 秒）**：深度功能演示，此时必须拆分为分步微动作，严禁全量静态停留。
