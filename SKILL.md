...
1. **确认范围与风格。** 已有工程沿用 brief 和用户决定，只处理本次修改。新片补齐：产品/仓库、内容范围（某次更新还是产品整体）、发布平台、时长与画幅、语言、是否允许链接，以及用户有没有参考视频。风格没定时问一次："沿用代码库自己的设计风格（推荐），还是默认的暖白／炭黑风格？"`repo` 用产品设计；`default` 用默认样式包装；`hybrid` 保留产品识别、调整外层排版。常见起点：横版、45–60 秒、中文。
...
- **真实功能。** 正式片至少一项可追溯的主张；每项记录发布状态、来源和白话解释。演示数据可以固定，功能效果与数字要有证据。
